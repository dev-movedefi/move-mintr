...
import MYContract from "./moveToken"
import SyntheticAssets from "./syntheticAssets"
...
async function lockedTOKENFor(synAddress, outDecimal = false) {
  ...
  let result = await offChainCall(synAssets, address, "lockedToken", [address])
  if(result !== null) {
    result = new Decimal(result).div(tokenDecimals)
...
async function verifyUnlock(synAddr, unlockTOKEN) {
  ...
  if(lockedAmount === null) {
    return "Can't get locked MOVE amount."
  }

  if(!lockedAmount.gte(unlockTOKEN)) {
    return "MOVE unlocked amount exceeds the locked."
  }
